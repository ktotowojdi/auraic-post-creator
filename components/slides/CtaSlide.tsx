...
export default function CtaSlide({ slide, index, total, interactive, scale = 0.44, onPositionChange, onDragStart, platform = 'instagram' }: Props) {
    ...
    >
      ...
      {wrap('text',
        <div className="w-[880px]">
          <p className="font-medium text-white leading-[1.4]" style={{ fontSize: getFontSize(slide, platform, 72) }}>
            <TextWithBreaks text={slide.text.primary} />
            {slide.text.secondary && (
              <>
                {' '}
                <span className="font-bold"><TextWithBreaks text={slide.text.secondary} /></span>
              </>
            )}
          </p>
        </div>
      ...
      {wrap('bottomIcon', <BottomIcon type={slide.bottomIcon || 'bookmark'} />)}
    </div>
  );
}
